...
export default function ProductDescription() {
    ...
    const getCurrentSizes = () => {
        switch (selectedFit) {
            case 'PLUS':
                return sampleProduct.sizes.plus;
            case 'PETITE':
                return sampleProduct.sizes.petite;
            default:
                return sampleProduct.sizes.standard;
        }
    };

    const accordionSections = {
        'Description': sampleProduct.description,
        'Product Details': sampleProduct.productDetails.join('\n'),
        'Shipping & Returns': sampleProduct.shippingReturns
    };
                ...
                <div className="container mx-auto">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-8">
                        ...
                        <div className="relative flex gap-2 w-full">
                            ...
                            <div className="hidden md:flex flex-col gap-2 w-20">
                                {sampleProduct.images.map((image, index) => (
                                    <div
                                        key={image.id}
                                        className={`relative aspect-[3/4] cursor-pointer overflow-hidden rounded-sm ${currentImageIndex === index
                                            ? 'ring-2 ring-black'
                            ...
                            <div className="flex-1 min-h-0">
                                <div className="relative aspect-[3/4] md:aspect-[2/3] w-full rounded-lg overflow-hidden">
                                    <Image
                                        src={sampleProduct.images[currentImageIndex].url}
                                        alt={sampleProduct.images[currentImageIndex].alt}
                                        fill
                                        className="object-contain"
                                        priority={true}
                        ...
                        <div className="space-y-8">
                            <div className="flex justify-between items-start">
                                <div>
                                    <h1 className="text-3xl font-light tracking-wide text-[#211E1E]">{sampleProduct.name}</h1>
                                    <div className="mt-4 space-y-2">
                                        <p className="text-2xl font-medium">${sampleProduct.price}</p>
                                        <p className="text-sm text-gray-600">
                                            4 interest-free payments of $82.00 with <span className="font-medium">Afterpay</span>
                                        </p>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-4">
                                <p className="text-sm font-medium text-[#211E1E]">COLOR: {sampleProduct.colors[0].name}</p>
                            ...
                            <div className="space-y-6">
                                <div className="flex border-b border-[#211E1E]">
                                    {['STANDARD', 'PLUS', 'PETITE'].map((fit) => (
                                        <button
                                            key={fit}
                                            className={`px-6 py-2.5 text-sm ${fit === selectedFit
                                ...
                                <div className="grid grid-cols-4 sm:grid-cols-7 gap-2">
                                    {getCurrentSizes().map(({ label, available }) => (
                                        <button
                                            key={label}
                                            disabled={!available}
                                            className={`py-3 rounded ${selectedSize === label
                                ...
                                {Object.entries(accordionSections).map(([section, content]) => (
                                    <div key={section} className="border-t border-[#211E1E]/10">
                                        ...
                                        >
                                            ...
                                            <ChevronDown
                                                className={`w-5 h-5 transition-transform duration-200 ${openSection === section ? 'rotate-180' : ''
                                                    }`}
                                            />
                                        </button>
                                        {openSection === section && (
                    ...
                    <div className="mt-20 mb-16">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                            {sampleProduct.recommendations.map((product) => (
