...
export async function POST(request: Request) {
  try {
    ...
    backendFormData.append("prompt", prompt as string);
    ...
    if (negativePrompt) {
      backendFormData.append("negative_prompt", negativePrompt as string);
    }
    ...
    console.log("Sending generative-fill request to backend");

    // Send the request to the backend server
    const backendResponse = await fetch(
      "http://127.0.0.1:8000/api/image/generative-fill",
      {
        method: "POST",
